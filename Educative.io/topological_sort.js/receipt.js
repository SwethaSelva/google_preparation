function findRecipes(recipes, ingredients, supplies) {
  ...
  let receipesInDegree = {}; // { rec: <count of ing>}
  for (let i = 0; i < ingredients.length; i++) {
    if (!receipesInDegree[recipes[i]]) receipesInDegree[recipes[i]] = ingredients[i].length || 0;
  ...
  for (let supply of supplies) {
    ...
    for (let rec of ingredientList[supply]) {
      receipesInDegree[rec] -= 1;
      if (!receipesInDegree[rec]) {
